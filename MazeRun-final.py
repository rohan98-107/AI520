...
def DFS(maze, root_x=0, root_y=0, stack=None):
    ...
    if not stack:
        ...
        prev = stack.pop()
    return DFS(maze, prev[0], prev[1], stack)
...
def A_star(maze, heuristic):
    ...
    while len(fringe_hp) > 0:
        ...
        i = min_fringe_elt[1][0]; j = min_fringe_elt[1][1]
        maze[i][j] = VISITED
        ...
        if (i == dim-1) & (j == dim-1):
            ...
            while (s >= 0) & (t >= 0):
                maze[s][t] = VISITED
                path.append((s,t))
                parent = parents[s][t]; s = parent[0]; t = parent[1]
            path.reverse()
        ...
        for dx, dy in dirs:
            ...
            if isValid(maze, child[0], child[1]) and child not in visited:
                ...
                path_length = 0
                parent_i = child[0]; parent_j = child[1]
                while (parent_i >= 0) & (parent_j >= 0):
                    parent = parents[parent_i][parent_j]
                    parent_i = parent[0]; parent_j = parent[1]
                    path_length += 1
...
def bdBFS(maze):
    ...
    # 1. retrace final path via parents matrix
    s_path = []
    parent_i = s_path_terminal[0]; parent_j = s_path_terminal[1]
    while (parent_i >= 0) & (parent_j >= 0):
        ...
        parent_j = parent[1]
    t_path = []
    parent_i = t_path_terminal[0]; parent_j = t_path_terminal[1]
    while (parent_i >= 0) & (parent_j >= 0):
...
dim = 15; wall_probability = 0.3; algo = 'BD_BFS'; num_trials=5
