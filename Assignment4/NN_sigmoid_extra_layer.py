...
def output_test(n,layer_1_weights,layer_2_weights, layer_3_weights):
    ...
    for i in range(m):
        ...
        layer_2 = sigmoid(np.matmul(layer_1_weights, flattened))
        layer_3 = sigmoid(np.matmul(layer_2_weights, layer_2))
        ...
        img.save("imgs/sigmoid-"+str(n)+image_files[i] +"-colored"+ '.png')
        img = Image.fromarray(gmats[i].reshape(img_size,img_size), 'L')
        img.save("imgs/sigmoid-"+str(n)+image_files[i] +"-grayscale"+ '.png')
