    ...
    while heap and parents[n-1][n-1] == (-1,-1):
        ...
        for dx, dy in dirs2:
            ...
            if isValid(maze, new_x, new_y) and parents[new_x][new_y] == (-1,-1):
                ...
                priority =  length + 1 + manhattan_distance_to_goal - weight * min_euclid_distance_to_nearest_fire(fires, new_x, new_y)
...
def smarterFireSurvivor(maze, q, weight = 0.5):
    ...
    while curr_x != n-1 or curr_y != n-1: #not at end
        ...
        maze, fires = simulateFire(maze, fires)
...
combinedFireDriver(dim=20, trials = 500, smarterFireFunction = smarterFireSurvivor, weight = 1)
